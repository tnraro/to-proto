...
- 서버·계정·업로드 없음, 모든 데이터는 브라우저 IndexedDB에 저장
- 고양이별 기록 (날짜/시간, 토 종류 다중 선택, 메모)
- 월간 캘린더 + 일별 상세
...
- `src/lib/db.ts` — raw IndexedDB 래퍼 (get/set)
- `src/lib/storage.ts` — 저장 계층 (IndexedDB)
- `src/lib/thresholds.ts` — 임계값 평가 로직
- `src/hooks/useStore.ts` — 상태 관리 (고양이/기록/규칙/경고 CRUD + hydration)
- `src/components/` — 기록 폼, 기록 리스트, 캘린더, 통계, 경고 규칙, 설정
